...
def read_squad(file: str) -> tuple[list[dict[str, object]], list[str]]:
    with open(file) as f:
        data = json.load(f)
...
def read_hotpotqa(file: str) -> tuple[list[dict[str, object]], list[str]]:
    with open(file) as f:
        data = json.load(f)
...
def generate_input_output(index: int, num_docs: int) -> tuple[str, list[str]]:
    curr_q = QAS[index]["query"]
    curr_a = QAS[index]["outputs"]
    curr_docs = QAS[index]["context"]
    curr_more = QAS[index].get("more_context", [])

    assert isinstance(curr_q, str)
    assert isinstance(curr_a, list)
    assert isinstance(curr_docs, list)
    assert isinstance(curr_more, list)

    if num_docs < len(DOCS):
        if (num_docs - len(curr_docs)) > len(curr_more):
            addition_docs = [i for i, d in enumerate(DOCS) if i not in curr_docs + curr_more]
            all_docs = (
                curr_docs + curr_more + random.sample(addition_docs, max(0, num_docs - len(curr_docs) - len(curr_more)))
            )
        else:
            all_docs = curr_docs + random.sample(curr_more, num_docs - len(curr_docs))

        all_docs_strs = [DOCS[idx] for idx in all_docs]
    else:
        all_docs_strs = DOCS

    random.Random(args.random_seed).shuffle(all_docs_strs)

    context = "\n\n".join([DOCUMENT_PROMPT.format(i=i + 1, document=d) for i, d in enumerate(all_docs_strs)])
...
def generate_samples(
    ...
    for index in tqdm(range(num_samples)):
        ...
        while True:
            try:
                ...
                if used_docs > incremental:
                    ...
                    # Can't shrink further; accept the input as-is and move
                    # on. Without this, the inner while-True loops forever
                    # when even ``incremental`` docs overflow max_seq_length
                    # (upstream NVIDIA bug).
                    break
        ...
        formatted_output = {"index": index, "input": input_text, "outputs": answer, "length": length}
...
def main() -> None:
    ...
    dump_jsonl(save_file, write_jsons)
