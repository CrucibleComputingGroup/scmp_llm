...
For every ``SCLinear`` we register hooks that, given the same input the SC
kernel saw, compute the ``F.linear`` reference and record MSE / max|Δ|.

Works for both llama and qwen (auto-dispatched via ``MODEL_PATH``). Each
side defines its own ``SCLinear`` class; we detect both via duck typing on
the class name.
"""
import os
import statistics
...
def make_hooks(name, weight, bias):
    ...
    def post(mod, args, output):
        ...
        # an empty input. Skip — there is no SC error to record for an
        # unused expert call.
        if x.numel() == 0:
            return
        with torch.no_grad():
...
# Detect SCLinear by class name so we don't have to import both llama and
# qwen modules (qwen3_sc has import-time side effects via module patching).
for name, mod in model.named_modules():
    if type(mod).__name__ == "SCLinear":
...
print(f"Captured {len(records)} SC matmul calls. model={MODEL_PATH}  "
      f"sc_prec={SC_PREC} stoc_len={SC_STOC_LEN} granularity={SC_ATTN_GRANULARITY}\n")

by_kind = defaultdict(list)
for r in records:
    short = r["name"].rsplit(".", 1)[-1]
...
print(f"{'layer.proj':<40}  {'shape':<24}  {'MSE':>11}  {'max|Δ|':>9}  {'|x|max':>8}")
print("-" * 110)
worst = sorted(records, key=lambda r: r["mse"], reverse=True)[:10]
for r in worst:
    print(f"{r['name']:<40}  {str(r['shape']):<24}  {r['mse']:11.4e}  "
